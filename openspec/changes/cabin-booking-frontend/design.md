...
| `VITE_WHATSAPP_NUMBER` | no | **When unset, the button is not rendered at all.** A dead `wa.me` link fails on the prospect's side, where nobody will ever report it. When the API gains the field, this becomes the fallback and is then deleted. |
