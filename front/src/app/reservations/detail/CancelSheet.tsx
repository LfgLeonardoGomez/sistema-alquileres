...
export function CancelSheet({ reservationId, checkIn, checkOut, cabinName, onClose }: Props) {
  ...
  return (
    <div role="dialog" aria-label={CANCEL_SHEET_COPY.title}>
      <h2>{CANCEL_SHEET_COPY.title}</h2>
      <p>{cancelConfirmationBody(formatDateRange(checkIn, checkOut), cabinName)}</p>

      {cancelReservation.isError ? <p role="alert">{resolveErrorCopy(cancelReservation.error)}</p> : null}
