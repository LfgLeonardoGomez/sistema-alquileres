...
import { Avatar, Button, rowClass, Sheet } from '../../shared/ui'
...
export function GuestSheet({ guest, reservations, cabins, onClose }: Props) {
  ...
  return (
    <Sheet>
      <div role="dialog" aria-label={guest.full_name} className="flex flex-col gap-[18px]">
        <div className="flex items-center gap-3.5">
          <Avatar name={guest.full_name} size={56} />
          <div className="flex flex-1 flex-col gap-0.5">
            <h2 className="text-2xl font-extrabold tracking-tight text-primary">{guest.full_name}</h2>
            <p className="text-lg text-muted">{guest.phone}</p>
          </div>
          {/* Phase 7b (owner-waived TDD, 2026-09-06): the "Editar" affordance
              ...
              reused in edit mode. */}
          <button type="button" className="text-[17px] font-bold text-accent-ink" onClick={() => setIsEditOpen(true)}>
            {GUEST_SHEET_COPY.edit}
          </button>
        </div>

        <section aria-label={GUEST_SHEET_COPY.saldoLabel} className="flex items-baseline justify-between rounded-row bg-warm-tint px-5 py-[18px]">
          <span className="text-lg font-bold text-warm">{GUEST_SHEET_COPY.saldoLabel}</span>
          <span className="text-[26px] font-extrabold text-warm">{formatMoney(summary.balanceCentavos)}</span>
        </section>

        <section aria-label={GUEST_SHEET_COPY.staysTitle} className="flex flex-col gap-2.5">
          <h3 className="text-[17px] font-extrabold text-muted">{GUEST_SHEET_COPY.staysTitle}</h3>
          <ul className="flex flex-col gap-2.5">
            {visibleStays.map((stay) => {
              const nights = nightsBetween(stay.checkIn, stay.checkOut)
              return (
                <li key={stay.id} className={rowClass}>
                  <Link to={`/reserva/${stay.id}`} className="flex flex-1 flex-col gap-0.5">
          ...
          {overflowStays.length > 0 ? (
            <p className="pl-1 text-base text-faint">{stayOverflowLine(overflowStays.length, monthNameFor(overflowStays[0]!.checkIn))}</p>
          ) : null}
        </section>
        ...
        <Link to="/reserva/nueva/1" className="flex h-16 items-center justify-center rounded-btn bg-accent text-lg font-extrabold text-white">
          {GUEST_SHEET_COPY.addReservation}
        </Link>
        <button type="button" className="text-center text-[17px] font-bold text-faint" onClick={() => setIsDeactivateOpen(true)}>
